...
/* ============================================================
   MOCK_STORE — unica fonte di verità. Per passare ai dati
   reali: sostituisci ogni MOCK_STORE.xxx con la chiamata API
   corrispondente e rimuovi questo blocco.
   ============================================================ */
const MOCK_STORE = (() => {
  const T = 10000;
  const RAW = [
    { symbol:'BTC',  name:'Bitcoin',     price:60772,   alloc:15, chg24h:-0.47, chg7d:-3.2,  type:'wallet',   mcap:1190000000000, exchange:null      },
    { symbol:'ETH',  name:'Ethereum',    price:1550.85, alloc:15, chg24h:-2.06, chg7d:-5.1,  type:'wallet',   mcap:186000000000,  exchange:null      },
    { symbol:'XRP',  name:'XRP',         price:1.0806,  alloc:15, chg24h:-1.57, chg7d: 2.1,  type:'wallet',   mcap:61000000000,   exchange:null      },
    { symbol:'QNT',  name:'Quant',       price:64.47,   alloc: 7, chg24h:-1.96, chg7d:-8.4,  type:'wallet',   mcap:780000000,     exchange:null      },
    { symbol:'TAO',  name:'Bittensor',   price:191.10,  alloc: 7, chg24h:-2.10, chg7d: 4.2,  type:'exchange', mcap:1290000000,    exchange:'Binance' },
    { symbol:'ADS',  name:'Adshares',    price:0.3274,  alloc: 7, chg24h:-7.83, chg7d:-12.1, type:'wallet',   mcap:32000000,      exchange:null      },
    { symbol:'XDC',  name:'XDC Network', price:0.02878, alloc: 7, chg24h:-1.84, chg7d:-3.6,  type:'wallet',   mcap:510000000,     exchange:null      },
    { symbol:'NXRA', name:'Nexera',      price:0.00195, alloc: 7, chg24h:-2.01, chg7d:-6.3,  type:'exchange', mcap:45000000,      exchange:'Kraken'  },
    { symbol:'TRAC', name:'OriginTrail', price:0.3274,  alloc: 5, chg24h:-7.83, chg7d:-9.2,  type:'wallet',   mcap:140000000,     exchange:null      },
    { symbol:'EWT',  name:'Energy Web',  price:0.26763, alloc: 5, chg24h: 0.43, chg7d: 1.8,  type:'wallet',   mcap:42000000,      exchange:null      },
    { symbol:'PEAQ', name:'Peaq',        price:0.02206, alloc: 5, chg24h:-2.09, chg7d:-4.7,  type:'exchange', mcap:180000000,     exchange:'Bybit'   },
    { symbol:'ATOM', name:'Cosmos',      price:1.603,   alloc: 5, chg24h:-3.32, chg7d:-7.1,  type:'wallet',   mcap:1900000000,    exchange:null      },
  ];
  ...
  return {
    ...
    totalValue,
    totalPnl24h,
    totalPnl24hPct: (totalPnl24h/totalValue)*100,
    onChainValue,
    exchValue:      totalValue - onChainValue,
    stableValue:    0,
    stablePct:      0,
    equityCurve: [
      {day:'01/06',value:10000},{day:'05/06',value:9820},{day:'10/06',value:9650},
      {day:'15/06',value:9480},{day:'20/06',value:9210},{day:'25/06',value:9080},
      {day:'Oggi', value:Math.round(totalValue)},
    ],
    transactions: [
      {id:1,type:'Deposito',asset:'USDT',qty:10000,amount:10000,price:1,      date:'01/06/2026',source:'Binance',hash:'0x1a2b3c',wallet:'Main',status:'Confermata',fee:'$2.10'},
      {id:2,type:'Trade',   asset:'BTC', qty:0.025, amount:1500, price:60729, date:'01/06/2026',source:'Binance',hash:'0x4d5e6f',wallet:'Main',status:'Confermata',fee:'$3.80'},
      {id:3,type:'Trade',   asset:'ETH', qty:0.967, amount:1500, price:1551,  date:'01/06/2026',source:'Binance',hash:'0x7g8h9i',wallet:'Main',status:'Confermata',fee:'$2.20'},
      {id:4,type:'Trade',   asset:'XRP', qty:1388,  amount:1500, price:1.081, date:'01/06/2026',source:'Kraken', hash:'0xab1cd2',wallet:'Main',status:'Confermata',fee:'$1.40'},
      {id:5,type:'Trade',   asset:'QNT', qty:10.86, amount:700,  price:64.46, date:'02/06/2026',source:'Wallet', hash:'0xef3gh4',wallet:'Cold',status:'Confermata',fee:'$0.80'},
      {id:6,type:'Trade',   asset:'TAO', qty:3.66,  amount:700,  price:191.3, date:'02/06/2026',source:'Binance',hash:'0xij5kl6',wallet:'Main',status:'Confermata',fee:'$1.20'},
      {id:7,type:'Trade',   asset:'ADS', qty:2138,  amount:700,  price:0.327, date:'02/06/2026',source:'Wallet', hash:'0xmn7op8',wallet:'Cold',status:'Confermata',fee:'$0.30'},
      {id:8,type:'Trade',   asset:'XDC', qty:24322, amount:700,  price:0.029, date:'03/06/2026',source:'Wallet', hash:'0xqr9st0',wallet:'Cold',status:'Confermata',fee:'$0.20'},
      {id:9,type:'Trade',   asset:'NXRA',qty:358974,amount:700,  price:0.002, date:'03/06/2026',source:'Kraken', hash:'0xuv1wx2',wallet:'Main',status:'Confermata',fee:'$1.10'},
    ],
    ...
    rebalanceTargets:{BTC:15,ETH:15,XRP:15,QNT:7,TAO:7,ADS:7,XDC:7,NXRA:7,TRAC:5,EWT:5,PEAQ:5,ATOM:5},
    perf:{twrr:-8.4,mwrr:-7.9,cagr:-18.2,roi7d:-4.8,roi30d:-8.4,roi90d:-12.1,roi1y:-8.4,bestAsset:'EWT',bestPct:0.43,worstAsset:'ADS',worstPct:-7.83,btcBench:-3.2,ethBench:-5.1,sp500Bench:1.2},
    risk:{sharpe:-0.62,var95:-4.8,var95Usd:-480,drawdown:-12.4,drawdownUsd:-1240,volatility:32.4,beta:0.91},
    comp:{
      herfindahl:0.12,stableRatio:0,stableUsd:0,
      onChainPct:+(onChainValue/totalValue*100).toFixed(1),
      cexPct:+((totalValue-onChainValue)/totalValue*100).toFixed(1),
      turnover:0,
      dca: RAW.slice(0,5).map(a=>({symbol:a.symbol,avgPrice:+(a.price*1.08).toFixed(4),currentPrice:a.price,pct:-7.4})),
    },
  };
})();
/* ============================================================ */
    ...
    <div className="space-y-4">
      ...
      <Glass C={C}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',height:110,overflow:'visible'}} preserveAspectRatio="none">
          ...
          <polyline points={sparkLine} fill="none" stroke={eqColor} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
    ...
    <div className="space-y-4">
      <Glass C={C}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',height:90,overflow:'visible'}} preserveAspectRatio="none">
          ...
          <polygon points={`0,${H} ${pts} ${W},${H}`} fill="url(#storGrad)"/>
          <polyline points={pts} fill="none" stroke={chartColor} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
        <div className="flex items-center justify-between mt-2 pt-3" style={{borderTop:`0.5px solid ${C.sep}`}}>
          {[
            {l:'Inizio',v:`$${equitySnap[0]?.value.toLocaleString()}`,c:C.secondary},
            {l:'Oggi',  v:`$${equitySnap[equitySnap.length-1]?.value.toLocaleString()}`,c:C.primary},
            {l:'Δ',     v:`${(((equitySnap[equitySnap.length-1]?.value-equitySnap[0]?.value)/equitySnap[0]?.value)*100).toFixed(1)}%`,
              c:equitySnap[equitySnap.length-1]?.value>=equitySnap[0]?.value?C.green:C.red},
          ].map((d,i)=>(
      ...
      <div className="grid grid-cols-3 gap-3">
        <Stat C={C} label="Depositato" value={`$${totalIn.toLocaleString()}`}   color={C.green}  sub="totale in entrata"/>
...
const MercatoView = ({C, settings}) => {
  ...
  const BTC_DOM=54.2, TOTAL_MCAP=3.21, FNG={value:28,label:'Paura',color:'#ef4444'};
  const ETH_BTC=(GLOBAL[1]?.price/GLOBAL[0]?.price).toFixed(5);
